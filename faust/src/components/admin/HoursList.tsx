...
import { hoursFormSchema, type AdminHours } from "@/schemas/settings";
    ...
    <li className={styles.row}>
      ...
      {!row.closed && (
        <div className={styles.times}>
          ...
          <label className={styles.closedToggle} title="Час закриття належить наступній календарній добі">
            <input
              type="checkbox"
...
export const HoursList = ({ hours }: { hours: AdminHours[] }) => (
  <ul className={styles.list}>
    {hours.map((entry) => (
      <HoursRow key={entry.id} hours={entry} />
    ))}
  </ul>
);
