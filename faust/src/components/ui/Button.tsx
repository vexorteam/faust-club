...
const OWN_KEYS = ["variant", "full", "showArrow", "children", "className"] as const;
...
const classNames = (variant: Variant, full: boolean | undefined, className: string | undefined) =>
  [styles.button, variant === "primary" ? styles.primary : styles.ghost, full && styles.full, className]
    .filter(Boolean)
    .join(" ");
...
export const Button = (props: ButtonProps) => {
  ...
  if (isLinkProps(props)) {
    ...
    return (
      <Link href={href} className={cls} {...externalProps} {...linkOwned}>
        {children}
  ...
  return (
    <button className={cls} {...buttonOwned}>
      {children}
      {showArrow && <IconArrowUpRight className={styles.icon} />}
    </button>
  );
