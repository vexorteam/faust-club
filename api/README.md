...
Адмінські ендпоінти — кроки Б5–Б6 (див. §13.5 у `CLAUDE.md`).
...
| `src/faust_api/routers/` | ендпоінти: `health`, публічні `menu`/`atmosphere`, `auth`; `admin/` — Б5–Б6 |
| `src/faust_api/schemas/` | Pydantic-моделі відповідей — це і є контракт §5.3 |
| `src/faust_api/services/` | те, чого не має знати роутер: URL фото, вебхук ревалідації, ліміт спроб входу |
| `migrations/` | Alembic |
